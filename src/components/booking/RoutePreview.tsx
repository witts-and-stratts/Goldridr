...
function FlightCard({
  ...
  const stats = [
    ...
    { label: 'Date', value: flight.flightDate },
    ...
    >
      <motion.div variants={v.group} className='p-5'>
        ...
        <motion.div variants={v.group} className='flex items-center'>
          <motion.span
            variants={v.fromLeft}
            className='font-wide text-3xl text-white'
          >
            {flight.departure}
          </motion.span>
          <div className='flex flex-1 items-center justify-center px-4'>
            <motion.div
              variants={v.line}
              className='relative h-px w-full bg-white/20'
            >
              <motion.span
                variants={v.plane}
        ...
        >
          <span>
            {flight.origin}
            {terminalLabel(flight.departureTerminal)}
          </span>
          <span>
            {flight.destination}
            {terminalLabel(flight.terminal)}
          </span>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        variants={v.line}
        className='mx-5 origin-left border-t border-dashed border-white/15'
      />

      <motion.div variants={v.group} className='flex justify-between gap-4 p-5'>
        {stats.map((stat) => (
...
function Stat({
  ...
  return (
    ...
    >
      ...
      <span className={cn('mt-1 block text-balance font-wide uppercase text-2xl leading-tight text-white xl:text-3xl', statClassName)}>
        {value}
      </span>
    </motion.div>
  );
...
export function RoutePreview({
  ...
  const distanceValue = distanceData
    ? `${Math.round(distanceData.total_miles)} mi`
    : '';
  const durationValue = isHourly
    ? hourlyHours
      ? `${hourlyHours} hr`
      : ''
    : (distanceData?.duration_text ?? '');
  ...
  return (
    <div className='relative isolate hidden min-h-[520px] overflow-hidden lg:flex lg:flex-col m-8 rounded-lg'>
      <AnimatePresence>
        {flightDetails && (
          <FlightCard flight={flightDetails} terminal={terminal} />
        )}
      </AnimatePresence>
