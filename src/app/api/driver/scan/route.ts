...
/**
 * Rider QR codes encode the booking verify link, e.g.
 * `https://goldridr.com/verify?reference=ABC123&email=rider@example.com`.
 * Plain references and JSON payloads are accepted as fallbacks so older
 * codes and manual entry keep working.
 */
function parseScanPayload( raw: string ): ScanPayload | null {
  ...
  try {
    ...
    if ( reference ) {
      return { reference, email: url.searchParams.get( "email" ) };
    }
  } catch {}
  ...
  if ( /^[A-Za-z0-9-]{4,32}$/.test( value ) ) {
    return { reference: value, email: null };
  }
  ...
  try {
    ...
    if ( !scan ) {
      return NextResponse.json(
        { success: false, error: "Unrecognized QR code" },
        { status: 400 }
      );
    }
    ...
    if ( scan.email && booking.email.toLowerCase() !== scan.email.trim().toLowerCase() ) {
      return NextResponse.json(
        { success: false, error: "QR code does not match the booking on file" },
        { status: 404 }
      );
    }
