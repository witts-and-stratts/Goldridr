...
export async function POST( request: Request, { params }: { params: Promise<{ token: string }> } ) {
  try {
    ...
    console.error( "Square checkout failed", cause );
    return NextResponse.json( { success: false, error: cause instanceof Error ? cause.message : "Square payment failed" }, { status: 502 } );
  }
}
